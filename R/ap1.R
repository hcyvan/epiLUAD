...
saveImage2("rna.ap1.target.barplot.pdf",width = 5,height = 3.5)
grid.arrange(p1,p2,p3,p4,nrow=2)
dev.off()
#----------------------------------------------------------------------------------------------------------------------
# Figure 6D. AP1 target
#----------------------------------------------------------------------------------------------------------------------
atacPeak<-AtacPeak()
rnaTPM<-RnaTPM()
getFeatures<-function(TF,chipseq){
  regioin<-readRDS(file.path(CONFIG$dataIntermediate, 'atac','homer.mask','tf.epiTFs.DAR.rds'))
  gr2<-bed2GRanges(loadData2(chipseq,header = FALSE))
  chipH3k27ac='D:/data/epiLUAD/GenomeBrowser/open/h3k27ac/A549/ENCFF614QOP.bed'
...
chipseqFOS='D:/data/epiLUAD/GenomeBrowser/open/chipseq/FOS/ENCFF459DPT.bed'
chipseqJUN='D:/data/epiLUAD/GenomeBrowser/open/chipseq/JUN/Serial/dex0/ENCFF280RQS.bed'
chipseqNKX21='D:/data/epiLUAD/GenomeBrowser/open/chipseq/GSE39998_RAW/NKX2_1.bed'
chipseqFOXA1='D:/data/epiLUAD/GenomeBrowser/open/chipseq/GSE39998_RAW/FOXA1.bed'
chipseqELF1='D:/data/epiLUAD/GenomeBrowser/open/chipseq/ELF1/ENCFF169QCM.bed'
chipseqETS1='D:/data/epiLUAD/GenomeBrowser/open/chipseq/ETS/ENCFF390XVX.bed'

featureFOS<-getFeatures('FOS', chipseqFOS)
featureJUN<-getFeatures('JUN', chipseqJUN)
featureAP1<-unique(union(featureFOS, featureJUN))
featureNKX21<-getFeatures('NKX2-1', chipseqNKX21)
featureFOXA1<-getFeatures('FOXA1', chipseqFOXA1)
featureELF1<-getFeatures('ELF1', chipseqELF1)
featureETS1<-getFeatures('ETS1', chipseqETS1)

saveImage2("rna.jun.target.circle.pdf",width = 5,height = 5)
plotFeatureRCircos(featureJUN)
dev.off()
saveImage2("rna.fos.target.circle.pdf",width = 8,height = 8)
plotFeatureRCircos(featureFOS)
dev.off()
...
saveImage2("rna.ets1.target.circle.pdf",width = 10,height = 10)
plotFeatureRCircos(featureNKX21)
dev.off()

saveImage2("rna.nkx21.target.circle.pdf",width = ,height = 8)
plotFeatureRCircos(featureNKX21)
dev.off()
#----------------------------------------------------------------------------------------------------------------------
# Figure 6F. Peak Accessibility/Methylation vs. gene expression TPM
#----------------------------------------------------------------------------------------------------------------------
atacPeak<-AtacPeak()
...
dev.off()
#----------------------------------------------------------------------------------------------------------------------
# Figure 4SF. Peak Accessibility/Methylation vs. gene expression TPM
#----------------------------------------------------------------------------------------------------------------------
atacPeak<-AtacPeak()
saveImage2("gene.access.methy.tpm.sfn.fam83a.perp.scatter.pdf",width = 3.5,height = 6)
par(mfrow=c(3,2))
atacPeak$plotCorAccessVsExpression('chr1:26862789-26863673', 'SFN',withTest = TRUE)#*
atacPeak$plotCorMethyVsExpression('chr1:26862789-26863673', 'SFN',withTest = TRUE)#*
atacPeak$plotCorAccessVsExpression('chr8:123181857-123183366', 'FAM83A',withTest = TRUE)
atacPeak$plotCorMethyVsExpression('chr8:123181857-123183366', 'FAM83A',withTest = TRUE)
atacPeak$plotCorAccessVsExpression('chr6:138088241-138088869', 'PERP',withTest = TRUE)#
atacPeak$plotCorMethyVsExpression('chr6:138088241-138088869', 'PERP',withTest = TRUE)#
dev.off()
#####################################################################################################
#####################################################################################################
#####################################################################################################
...
#####################################################################################################
special<-epiTFs$tf
special<-c(special,unlist(mapkGenes))%>%unique()
survival<-Survival()
srdeg<-Srdeg()
...
genes<-diff$symbol%>%sort%>%unique()
names(genes)<-NULL
intersect(special,genes)
survival$pickSurvival(genes)
survival$pickSurvivalDeg(genes)

# EFNA5 SDCBP PERP
atacPeak$plotCorAccessVsExpression('chr5:107474477-107475846', 'EFNA5',withTest = TRUE)#
atacPeak$plotCorMethyVsExpression('chr5:107474477-107475846', 'EFNA5',withTest = TRUE)#
atacPeak$plotCorAccessVsExpression('chr8:58552477-58554213', 'SDCBP',withTest = TRUE)#
atacPeak$plotCorMethyVsExpression('chr8:58552477-58554213', 'SDCBP',withTest = TRUE)#


diff[diff$symbol=='EFNA5',]



features<-featureNKX21
anno<-annoPeak(bed2GRanges(feature2Bed(features)))
diff<-anno[anno$symbol%in%srdeg$all()]
genes<-diff$symbol%>%sort%>%unique()
names(genes)<-NULL
